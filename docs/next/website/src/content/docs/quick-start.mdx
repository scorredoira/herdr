...
Ctrl-click opens pane links when your terminal sends the modified click to Herdr. This works for OSC 8 hyperlinks, visible `http://` or `https://` URLs, and file paths a plugin link handler claims. Holding the modifier underlines the link under the pointer as you move it, so you see what a click would take before you click it. `ui.link_click_modifier` changes which modifier that is. On macOS, use Ctrl-click for Herdr-handled pane links while mouse capture is enabled; Cmd-click is only available through the terminal-native bypass path, such as Shift-Cmd-click or `ui.mouse_capture = false`.
